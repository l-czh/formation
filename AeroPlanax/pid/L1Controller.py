...
# import pid as pid
from .utils import parse_config, get_diff_angle, get_length, get_vector_dot, get_cross_error, wrap_PI
device = "cuda:0"


class L1Controller:
    def __init__(self, config='l1controller', dt=0.1, n=1):
        ...
        self.current_loc = jnp.zeros((self.n, 2))
        self.ground_speed = jnp.zeros((self.n, 2))
        self.L1_xtrack_i = jnp.zeros((self.n, 1))
        self.L1_dist = jnp.zeros((self.n, 1))
        self.target_bearing = jnp.zeros((self.n, 1)) # 当前位置到目标点航向角/rad
        self.crosstrack_error = jnp.zeros((self.n, 1)) # 当前位置到航线的垂线长度
        self.nav_bearing = jnp.zeros((self.n, 1))
        self.Nu = jnp.zeros((self.n, 1))
        self.last_Nu = jnp.zeros((self.n, 1))
        self.latAccDem = jnp.zeros((self.n, 1)) # 横向加速度/ft/s^2
        self.bearing_error = jnp.zeros((self.n, 1))
        self.WPcircle = jnp.zeros((self.n, 1), dtype=bool)
    ...
    def prevent_indecision(self, yaw):
        Nu_limit = 0.9 * jnp.pi
        mask1 = (jnp.abs(self.Nu) > Nu_limit) & (jnp.abs(self.last_Nu) > Nu_limit) &\
            (jnp.abs(wrap_PI(self.target_bearing - yaw)) > (2 * jnp.pi / 3)) & ((self.Nu * self.last_Nu) < 0)
    ...
    def loiter_radius(self, radius, eas2tas, TAS_dem):
        ...
        lateral_accel_sea_level = math.tan(sanitized_bank_limit * jnp.pi / 180) * self.gravity
        ...
        mask1 = (abs(sanitized_bank_limit) < 1e-6) | (jnp.abs(nominal_velocity_sea_level) < 1e-6) |\
        ...
        mask3 = ~(mask1 | mask2)
        result += jnp.maximum(sea_level_radius * eas2tas_sq, radius) * mask3
        return result
    ...
    def update_waypoint(self, prev_WP, next_WP, dist_min, state, estate):
        ...
        # 0.3183099 = 1 / pi
        self.L1_dist = jnp.maximum(self.L1_damping * self.L1_period * groundSpeed / jnp.pi, dist_min * jnp.ones((self.n, 1)))
        ...
        AB = AB * ~mask1 + jnp.hstack((jnp.cos(yaw), jnp.sin(yaw))) * mask1
        AB_length = get_length(AB)
        AB = AB / AB_length
        ...
        mask1 = (WP_A_dist > self.L1_dist) & ((alongTrackDist / jnp.maximum(WP_A_dist, jnp.ones(self.n, 1))) < -0.7071)
        ...
        self.Nu = jnp.atan2(xtrackVel, ltrackVel) * mask1
        # bearing (radians) from AC to L1 point
        self.nav_bearing = jnp.atan2(-A_air_unit[:, 1] , -A_air_unit[:, 0]).reshape(-1, 1) * mask1
        ...
        self.Nu += jnp.atan2(xtrackVel, ltrackVel) * mask2
        # bearing (radians) from AC to L1 point
        self.nav_bearing += jnp.atan2(-B_air_unit[:, 1], -B_air_unit[:, 0]).reshape(-1, 1) * mask2
        mask3 = ~(mask1 | mask2)
        ...
        Nu2 = jnp.atan2(xtrackVel,ltrackVel)
        # Calculate Nu1 angle (Angle to L1 reference point)
        sine_Nu1 = self.crosstrack_error / jnp.maximum(self.L1_dist, 0.1 * jnp.ones((self.n, 1)))
        # Limit sine of Nu1 to provide a controlled track capture angle of 45 deg
        sine_Nu1 = jnp.clip(sine_Nu1, -0.7071, 0.7071)
        Nu1 = jnp.asin(sine_Nu1)
        ...
        mask1 = jnp.abs(Nu1) < (5 * jnp.pi / 180)
        self.L1_xtrack_i += Nu1 * self.L1_xtrack_i_gain * self.dt * mask1
        self.L1_xtrack_i = jnp.clip(self.L1_xtrack_i, -0.1, 0.1)
        # to converge to zero we must push Nu1 harder
        Nu1 += self.L1_xtrack_i
        self.Nu += (Nu1 + Nu2) * mask3
        # bearing (radians) from AC to L1 point
        self.nav_bearing += wrap_PI(jnp.atan2(AB[:, 1], AB[:, 0]).reshape(-1, 1) + Nu1) * mask3
        self.prevent_indecision(state)
        self.last_Nu = self.Nu
        # Limit Nu to +-(pi/2)
        self.Nu = jnp.clip(self.Nu, -jnp.pi / 2, jnp.pi / 2)
        self.latAccDem = K_L1 * groundSpeed * groundSpeed / self.L1_dist * jnp.sin(self.Nu)
    ...
    def update_loiter(self, center_WP, radius, loiter_direction, env, TAS_dem):
        ...
        # Get current position and velocity
        self.current_loc = jnp.hstack((npos.reshape(-1, 1), epos.reshape(-1, 1)))
        self.ground_speed = jnp.hstack((vx.reshape(-1, 1), vy.reshape(-1, 1)))
        groundSpeed = get_length(self.ground_speed)
        # update target_bearing
        ...
        # Calculate the L1 length required for specified period
        # 0.3183099 = 1/pi
        self.L1_dist = self.L1_damping * self.L1_period * groundSpeed / jnp.pi
        # Calculate the NE position of the aircraft relative to WP A
        ...
        A_air_unit += jnp.hstack((jnp.cos(yaw), jnp.sin(yaw))) * mask2
        mask3 = ~(mask1 | mask2)
        ...
        self.Nu = jnp.atan2(xtrackVelCap, ltrackVelCap)
        self.prevent_indecision(yaw)
        self.last_Nu = self.Nu
        self.Nu = jnp.clip(self.Nu, -jnp.pi / 2, jnp.pi / 2) # Limit Nu to +- Pi/2
        # Calculate lat accln demand to capture center_WP (use L1 guidance law)
        latAccDemCap = K_L1 * groundSpeed * groundSpeed / self.L1_dist * jnp.sin(self.Nu)
        ...
        latAccDemCircPD =  jnp.maximum(latAccDemCircPD, jnp.zeros((self.n, 1))) * mask1 + latAccDemCircPD * ~mask1
        # Calculate centripetal acceleration demand
        latAccDemCircCtr = velTangent * velTangent / jnp.maximum((0.5 * radius), (radius + xtrackErrCirc))
        ...
        # bearing (radians) from AC to L1 point
        A_air_unit = A_air / get_length(A_air) # Recompute A_air_unit if needed or ensure it's available
        self.nav_bearing = jnp.atan2(-A_air_unit[:, 1], -A_air_unit[:, 0]).reshape(-1, 1)
    ...
    def update_heading_hold(self, navigation_heading, env):
        ...
        self.WPcircle = jnp.zeros((self.n, 1), dtype=bool)
        self.crosstrack_error = jnp.zeros((self.n, 1))
        self.bearing_error = self.Nu # bearing error angle (radians), +ve to left of track
        # Limit Nu to +-pi
        self.Nu = jnp.clip(self.Nu, -jnp.pi / 2, jnp.pi / 2)
        self.latAccDem = 2 * jnp.sin(self.Nu) * VomegaA
    ...
    def update_level_flight(self, yaw):
        ...
        # Waypoint capture status is always false during heading hold
        self.WPcircle = jnp.zeros((self.n, 1), dtype=bool)
        self.latAccDem = jnp.zeros((self.n, 1))
    
    # return the bank angle needed to achieve tracking from the last update_*() operation
    def nav_roll(self, pitch):
        pitch = pitch.reshape(-1, 1)
        result = jnp.cos(pitch) * jnp.arctan(self.latAccDem / self.gravity)
        result = jnp.clip(result, -jnp.pi / 2, jnp.pi / 2)
        return result
